...
> **Status (corrected 2026-06-24, verified against code, not assumed):** The "do not implement
> until ORC ACADEMY v3 lands" gate was satisfied back on 2026-06-17 (v3 completed, see
> `docs/ROADMAP.md`) -- this header was simply never updated afterward. Actual state: **Phases
> 0-3 implemented** (`IModelRuntime`/`OllamaRuntime`/`LlamaCppServerRuntime`/`LLamaSharpRuntime`/
> `ModelDepot`/`SessionManager`/`AdapterManager`/`RuntimeOrchestrator` all real, not stubs --
> Phase 2's native generation re-verified on real hardware tonight, 67.7 tok/s on an RTX 4060,
> see `feat/native-runtime-orcchat`'s branch history). **Phase 4 (`OrcScheduler`) is also real and
> wired in**, not the stub this doc's phase table below implies -- `RuntimeOrchestrator.
> EnsureAdmitted` calls `TryAdmit` on every role admission with generation-tagged per-role VRAM
> accounting (see `OrcScheduler.cs`'s own corrected doc comment). Phase 5 (KV-cache optimization
...
Estimate/Risk columns are the original planning-time projections, kept verbatim for history.
Per the corrected status note at the top of this doc, **Phases 0-4 are all actually implemented**
as of 2026-06-24 -- the table below is no longer a forward plan for those rows, only Phase 5 is.

| Phase | Scope | Est. (solo) | Risk |
|---|---|---|---|
| **0 — Abstraction** (IMPLEMENTED) | `IModelRuntime` + `OllamaRuntime` + records; migrate AgentLoop; one test; DI decision per §4. Zero behavior change. | 2–3 days | Low |
| **1 — Server bridge** (IMPLEMENTED) | `LlamaCppServerRuntime` that **wraps existing `LlamaServerManager`** + `OllamaClient(Backend=LlamaCpp)`. Mostly wiring; proves the abstraction across two backends. | 2–4 days | Low |
| **2 — Native in-process** (IMPLEMENTED, GPU-verified) | `LLamaSharpRuntime : ILocalModelRuntime`. Direct GGUF + LoRA load, streaming, stats. Backend package detection (Cuda12/Cpu) at first-run/install. The real "no Ollama dependency" win. | 1–2 weeks | Medium |
| **3 — Orchestration** (IMPLEMENTED) | `ModelDepot` (registry + first-run downloader), `SessionManager` (persistent base model in VRAM), `AdapterManager` (boss/worker/reviewer LoRAs), telemetry (TTFT, tok/s, VRAM). Installer first-run UX is the long pole. | 3–5 weeks | Medium-High |
| **4 — Swarm scheduling** (IMPLEMENTED, wired into RuntimeOrchestrator) | `OrcScheduler` — capability + VRAM + lane-aware dispatch; pipeline boss→workers (prefill workers while user reviews plan). | 2–3 weeks | High |
| **5 — Optimization (research)** (not started) | Prefix KV cache for the shared warband/system prompt; multi-LoRA cache experiments. **Not blocking. Treat as research.** | open | Research |
