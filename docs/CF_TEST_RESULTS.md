...
| 14 | qwen2.5-coder-7b-instruct-q5_k_m | **120 (full held-out set)** | merged master (`8efa7fd8`: Tiers 1+1.5+2+2.5, SeqMax fix, thinking-suppression fix — none of the last two apply to this non-reasoning, plain-transformer model) | 29/120 (24.2%) | 118/128 (92.2%) | 130/203 (64.0%) | **16** (2 incidents) | **First 120-question run for this model, and the first on NEWCOREPC** (prior qwen2.5-coder data was 100-question runs on HARDCOREPC/HARDCORELAPTOPMSI, both pre-Tier-1/1.5/2.5). **Not a clean, zero-infra baseline** — 2 separate `NoKvSlot` incidents on the Reviewer role (both `minted=6`, `find_slot`/`decode: failed to find a memory slot for batch of size 1`, all 8 retries failing before a forced recycle), 16 total retry-failure lines. Both auto-recovered via the existing `ForceRecycle`-on-`NoKvSlot` mechanism (§7a) — nothing like the historical Gemma-4 near-total-crash pattern (216/223 attempts) — but whether the specific in-flight decode calls that triggered them ultimately succeeded or contributed a failed question is not yet traced; treat the B3 score below as *likely* still representative (2 incidents out of many hundreds of calls across 120 questions) rather than confirmed clean. **Full B0-B3 comparison: B0 16/120, B1 29/120, B2 52/120, B3 29/120 — B3 does NOT beat B2's top-k RAG baseline** — consistent with the pre-Tier-1 HARDCOREPC/HARDCORELAPTOPMSI findings that "the fabric does not yet beat RAG on this smaller model," now confirmed at full 120-question scale and after all the retrieval-quality tiers that later let Meta-Llama pull ahead of its own RAG baseline (run #7 above). Under the Remediation-Phase-2 `Graded capability` gate (PR #59, not yet in effect for this run's own report — evaluated by hand against these B0-B3 counts) this would still be a NO-GO: B3 loses to B2, not just to the 100%-pass-rate stretch goal. `boundary_stitch_pass_rate` **1/2 (50%)** and `segment_terminal_coverage` 118/128 (92.2%) both below target. `citation_precision` 64.0% is well under the 0.90 gate — a real, model-quality citation-discipline gap, not an infra artifact (gate `cf7_gate_20260715_163502`, ran 28:24 against a hardlinked `Models-CF7` copy after the shared depot's copy was found `.gguf.disabled`) |
| 15 | Qwen3.5-9B-Q8_0 | **120 (full held-out set)** | fully-fixed master post Phases A-D (`d43ac961`) | 14/120 (11.7%) | fail (blocking) | 100.0% (very few citations attempted) | **96 incidents, limited recovery** | **INVALID — corrupted-run artifact, not a capability measurement.** The first affected B3 `Reviewer` request's fully templated prompt plus requested completion exceeded its 8,192-cell attention context because the `chars/4` pack estimate under-counted Qwen's actual tokens. It generated 6 completion tokens before `failed to prepare attention ubatches`/`NoKvSlot`; forced recycling then retried the same oversized request. Recovery was limited, not absent: post-onset executors were `89× minted=1`, `3×2`, `3×3`, and `1×4` (12 additional successful conversations). See [CONTEXT_FABRIC_BUG_HISTORY.md §7e](CONTEXT_FABRIC_BUG_HISTORY.md) (resolved, live-validated — see row 17 for the valid replacement result). B3 14/120 remains unusable as a capability measurement. Gate `cf7_gate_20260715_181445_828_9ffa4cbe4b2b452a84ebf79df54096f0`, ran 30:50 |
| 16 | Qwen3.5-9B-Q4_K_M | **120 (full held-out set)**, run as a diagnostic for row 15 | fully-fixed master post Phases A-D (`d43ac961`), same code, smaller quant (~half Q8_0's VRAM footprint) | 40/120 (33.3%) | fail (blocking) | 100.0% (very few citations attempted) | **59 incidents, partial recovery** | **INVALID — corrupted-run artifact, same §7e capacity defect.** The first affected B3 `Reviewer` request arrived after about 168 prior `Researcher` conversations (the old ~144 estimate omitted conversations already served by the active executor) and generated 126 completion tokens, including an answer and partial citations, before exhausting its own attention context. The smaller quant's different recovery distribution does not establish VRAM fragmentation; allocations succeeded and later contexts ran cleanly. B3 40/120 remains unusable as a capability measurement. Gate `cf7_gate_20260715_184324_860_e9bb906ef66f497fa61936b4b30f8035`, ran 24:15 |
| 17 | Qwen3.5-9B-Q8_0 | **120 (full held-out set)** | §7e exact-token-admission fix (PR #63 head `33d6eab2`) | **75/120 (62.5%)** | 123/128 (96.1%) | 228/246 (92.7%) | **0** | **First valid full-scale Qwen3.5 measurement, and the highest B3 score recorded on any model to date** (previous best: Meta-Llama's 58/120, row 9). Zero `NoKvSlot`/memory-slot/attention-ubatch incidents across the whole run — 12 executor recycles, all healthy natural threshold recycles, far past the ~168-conversation onset point of the corrupted rows 15-16. Full B0-B3: B0 11/120, B1 31/120, B2 52/120, **B3 75/120 — `Graded capability` gate PASS** (B3 beats B2 by 23; citation_precision 92.7% clears 0.90). `boundary_stitch` 2/2 PASS; `max_prompt_tokens` 6,656/8,192 (the exact-token clamp visibly holding headroom). Overall verdict NO-GO **solely** on `segment_terminal_coverage` 123/128 — the known ingestion-quality gap, not an infra or capability failure. Ran 41:42 (much faster than the §7b-era 2h35m timings, which predated §7c's thinking suppression). Gate `cf7_gate_20260716_064147_541_b6bb535228404553804e035353c4e4d1`, log `cf7-7e-full120-q8_0-20260715_230005.log`; preceded by a clean 1-question smoke replay (`cf7-7e-smoke1-q8_0-20260715_224107.log`, 18:14, also zero incidents) |
| 18 | Qwen3.5-9B-Q4_K_M | **120 (full held-out set)** | §7e exact-token-admission fix (PR #63 head `33d6eab2`), same code as row 17 | **67/120 (55.8%)** | 117/128 (91.4%) | 302/313 (96.5%) | **0** | **Second valid Qwen3.5 measurement, confirming row 17 on the smaller quant.** Zero incidents. Full B0-B3: B0 12/120, B1 31/120, B2 57/120, **B3 67/120 — `Graded capability` gate PASS** (B3 beats B2 by 10; citation_precision 96.5%). `boundary_stitch` 2/2 PASS; `max_prompt_tokens` 6,655/8,192. Overall NO-GO solely on `segment_terminal_coverage` 117/128. Quantization cost vs. Q8_0 is visible but moderate (B3 75→67, coverage 123→117) while citation precision actually improved. Ran 27:51. Gate `cf7_gate_20260716_071035_006_dc4e73df346e46869d2fbd05da9355ab`, log `cf7-7e-full120-q4km-20260715_234244.log` |
...
- **The near-total-crash `NoKvSlot` pattern (216/223 attempts) is Gemma-4-specific**, a
  currently-unpatched upstream `llama.cpp` limitation, not a bug in TheOrc's own code —
  proven via direct A/B testing that ruled out both of our own leading theories
  (cross-conversation exhaustion, SWA-cache sizing). **Narrowed, not retracted, by newer
  evidence**: the qwen2.5-coder 120-question NEWCOREPC run (§6, row 14) hit 2 isolated
  `NoKvSlot` incidents (16 retry-failure lines total), both auto-recovered by the existing
  `ForceRecycle` safety net — a plain-transformer, non-Gemma model, at larger (120 vs. 100)
  question-set scale than the "zero crashes" claim above was measured at. This is nowhere
  near the Gemma-4 catastrophic pattern, but it means "NoKvSlot never happens on non-Gemma-4
  models" was an overstatement — the honest claim is "Gemma-4 uniquely makes it
  catastrophic and near-universal; other models can still hit it occasionally at scale,
  recoverably."
- **For runs already demonstrated infrastructure-clean**, the remaining known gaps are
  model-quality questions (citation discipline and segment-acceptance rate), a fundamentally
  different category from §7e's now-root-caused request-capacity defect.
...
- **Rows 15-16 remain invalid historical artifacts, but Qwen3.5-9B now has valid scores
  (rows 17-18).** §7e was root-caused as request-level attention-context exhaustion, fixed by
  exact-token admission (PR #63), and live-validated on 2026-07-16: both quants completed
  clean full 120-question gates with zero `NoKvSlot` incidents. The "16/120 Q8_0, 1/120
  Q4_K_M" numbers were invalidated by the thinking-suppression bug; the 14/120 and 40/120
  numbers were invalidated by §7e; the valid numbers are **Q8_0 B3 75/120 and Q4_K_M B3
  67/120**, both passing the `Graded capability` gate against their own B2 RAG baselines
  (52 and 57 respectively).
- **Qwen3.5-9B-Q8_0 is the strongest Context Fabric model measured to date** (row 17:
  B3 75/120, +23 over its RAG baseline, 92.7% citation precision) — overtaking
  Meta-Llama-3.1-8B (row 13: 58/120, +12, 90.2%). Unlike qwen2.5-coder (which loses to its
  own RAG baseline), the fabric's advantage transfers strongly to this model family. The
  binding constraint for a GO verdict is now `segment_terminal_coverage` on all models —
  an ingestion-quality problem, not an answering-quality one.

## 8. Phase E retrospective: new gates applied by hand to existing runs

The Remediation Phase 2/3 work (PRs #59/#61: `IsBlocking` plumbing, the `Graded capability`
gate, evidence budget telemetry, exhaustive-heuristic hardening) changed only
scoring/gate-evaluation code, not B0-B3 answer generation. Neither model below was re-run —
these verdicts are the new gate logic applied by hand to the B0-B3 counts already on record,
so a fresh `cf7-gate-expanded` invocation would reproduce them exactly (modulo the usual
run-to-run LLM sampling noise).

**`qwen2.5-coder-7b-instruct-q5_k_m`** (§6, row 14: B0 16/120, B1 29/120, B2 52/120,
B3 29/120, citation_precision 130/203 = 64.0%): `Graded capability` = **FAIL** — B3 (29)
does not exceed the best baseline, B2 (52). This is a genuine capability gap, not an
artifact of the old all-or-nothing `question_pass_rate` gate — the same conclusion an
exhaustive-all-questions-pass standard would have reached, just for the right reason this
time. `citation_precision` also fails its 0.90 blocking target at 64.0%. `ReadyForExpansion`
= **NO-GO** on multiple independent blocking gates.

**`Meta-Llama-3.1-8B-Instruct`** (§6, row 13, the final Tier 2.5 state: B2 46/120 per row 9 —
unaffected by the Tier 2.5 B3-only changes in rows 10-13 — B3 58/120, citation_precision
147/163 = 90.2%; B0/B1 were never logged for this specific 120-question run, but every other
run in this log shows B0 < B1 < B2, so B2 is confidently the binding baseline comparison):
`Graded capability` = **PASS** — B3 (58) beats B2 (46) by 12 questions, and
citation_precision (90.2%) clears the 0.90 target. Under the old all-or-nothing standard
this run would have reported NO-GO for the wrong reason (48.3% raw pass rate); the graded
gate correctly recognizes it as a real capability win over both baselines.
...
either the pre-Phase-2 all-or-nothing gate or an ungated summary would have given.**
