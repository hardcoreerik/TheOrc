...
| 15 | Qwen3.5-9B-Q8_0 | **120 (full held-out set)** | fully-fixed master post Phases A-D (`d43ac961`) | 14/120 (11.7%) | fail (blocking) | 100.0% (very few citations attempted) | **96 incidents, never recovered** | **INVALID — corrupted-run artifact, not a capability measurement.** A new, previously-undocumented `NoKvSlot` storm hit the `Reviewer` role after ~168 clean prior conversations and never recovered for the rest of the run (96 forced recycles straight, every one still failing at `minted=1`) — see [CONTEXT_FABRIC_BUG_HISTORY.md §7e](CONTEXT_FABRIC_BUG_HISTORY.md) (open, unresolved) for the full signature and why it's distinct from §7a/§7b/§7c. Directly contradicts §7b's "zero `NoKvSlot`, verified live" claim on the same code — that claim is downgraded to "greatly reduced, not deterministic." B3 (14) scoring below B2 RAG (52) and even plausibly below B0 is consistent with a broken run, not a real Qwen3.5 capability result. B0/B1/B2 baselines from this run (B0 unknown/not needed, B2 52/120) remain valid since baselines don't touch the `Reviewer` role's degraded state the same way; only B3's numbers are suspect. Gate `cf7_gate_20260715_181445_828_9ffa4cbe4b2b452a84ebf79df54096f0`, ran 30:50 (much faster than the ~2h35m clean-run baseline in §7b — itself another sign this run short-circuited rather than completing normally) |
| 16 | Qwen3.5-9B-Q4_K_M | **120 (full held-out set)**, run as a diagnostic for row 15 | fully-fixed master post Phases A-D (`d43ac961`), same code, smaller quant (~half Q8_0's VRAM footprint) | 40/120 (33.3%) | fail (blocking) | 100.0% (very few citations attempted) | **59 incidents, partial recovery** | **INVALID — corrupted-run artifact, same §7e storm, less severe.** Deliberately re-ran the identical suite on the smaller quant to test whether VRAM headroom is the deciding factor in row 15's storm. Result: storm still occurred (started at the same point, after `Researcher`'s 6th natural recycle, ~144 conversations in — essentially the same onset as Q8_0's ~168) but **partially recovered** rather than permanently breaking — `RECYCLING minted=` values after onset were `1, 1, 3, 1, 1, 4, 2, 2, 3, ...` (some fresh executors survived several conversations before failing again, unlike Q8_0's permanent `minted=1` every time). Consistent with — but not proof of — a VRAM-fragmentation root cause; narrows the open investigation without closing it (see §7e). B3 40/120 still loses to B2 RAG (57/120); not usable as a capability measurement for the same reason as row 15. Gate `cf7_gate_20260715_184324_860_e9bb906ef66f497fa61936b4b30f8035`, ran 24:15 (again much faster than the historical ~2h31m clean-run baseline, another short-circuit signal) |
...
- **§7b's "zero `NoKvSlot`" claim for Qwen3.5 does not hold up** (§6, rows 15-16): two fresh
  Phase E full-120-question runs on fully-fixed master, both quants, both hit a severe,
  previously-undocumented `NoKvSlot` storm (96 and 59 incidents respectively) that neither
  §7a's, §7b's, nor §7c's fixes prevent — see [CONTEXT_FABRIC_BUG_HISTORY.md
  §7e](CONTEXT_FABRIC_BUG_HISTORY.md) (open, unresolved). Both runs' B3 scores are void as
  capability measurements. Qwen3.5-9B **still has no valid CF-7 result** on this codebase —
  the "16/120 Q8_0, 1/120 Q4_K_M" numbers §7c reported were invalidated by the thinking-
  suppression bug it found, and these newer 14/120 and 40/120 numbers are invalidated by
  §7e. A clean, trustworthy Qwen3.5 measurement remains an open item, blocked on §7e's root
  cause.
