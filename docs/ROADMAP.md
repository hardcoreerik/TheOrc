...
### Installer: cross-platform Avalonia rewrite — Phase 1 shipped 2026-06-21
Full spec: [`INSTALLER_REVAMP_SPEC.md`](INSTALLER_REVAMP_SPEC.md). `OrchestratorSetup` was a Windows-only WPF wizard (`net10.0-windows`, WMI, `netsh`, `.lnk`, registry) built before the Avalonia migration, capping the whole product to Windows even though the app + daemon are cross-platform. **Phase 1 (UI port) is done**: `OrchestratorSetup` is now `net10.0` Avalonia, ported directly to the final 8-page flow (10 pages → 8: Profile dropped — the app's FirstRunWindow already does `.agent.md` — License folded into Welcome, OllamaCheck replaced by a native-first Runtime Setup page); a "Configure HIVE Ports" button added to Complete. All Windows-only service logic carries over unchanged (it was already plain C#, not WPF). **Remaining**: Phases 2-5 — extract an `IPlatformInstaller` interface, Linux impl, macOS impl + packaging. Firewall opens HIVE ports automatically with elevation (UAC/sudo), never manual-first, once those land.
