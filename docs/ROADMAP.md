...
| **3** | `ModelDepot` (local registry first; downloader later), `SessionManager` (persistent base model), `AdapterManager` (boss/worker/reviewer LoRAs), telemetry. | 🔶 Started — local-only ModelDepot registry and SessionManager base-load coordinator landed; first telemetry surface landed in Settings (Ollama-only, via `OllamaRuntime` — not SessionManager-driven, since Ollama has no persistent local session to manage); AdapterManager and SessionManager-backed telemetry pending. | Med-High |
