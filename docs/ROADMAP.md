...
The honest gaps: the Reviewer Quality Gate is advisory-only (can always be overridden), the Tool Editor hot-reload is a stub, and HIVE MIND multi-step tool calling on remote workers is Phase 3B (not yet built). Native Avalonia modal dialogs shipped partially in v1.8; the remaining dialogs (`AgentBuilderDialog`, `ModelWikiWindow`, `LabWindow`) are v1.9 scope. Native Runtime groundwork is now real in code (IModelRuntime, Ollama wrapper, llama.cpp server wrapper, LLamaSharp runtime, shared text tool-call parser, Chat/Swarm/HIVE worker/reviewer migration, ModelDepot, SessionManager, AdapterManager with per-role persistent LoRA contexts, RuntimeOrchestrator wiring all three together, two Settings-panel telemetry surfaces — OllamaRuntime health/stats and a ModelDepot local-folder scan — plus an experimental Avalonia-only native smoke surface with explicit Ollama fallback and local evidence capture, and a first OrcScheduler VRAM-budget admission check), but it is **not** production/default; runtime selection (actually routing a live call site through this stack instead of Ollama), SessionManager/AdapterManager-backed telemetry, OrcScheduler wiring into AdapterManager, and the SwarmSession Ollama eviction escape hatch remain future work. Everything else on the shipped list below is real, wired, and tested.
...
| **3** | `ModelDepot` (local registry first; downloader later), `SessionManager` (persistent base model), `AdapterManager` (boss/worker/reviewer LoRAs), telemetry. | 🔶 Near-closed — ModelDepot, SessionManager, AdapterManager (per-role persistent executors, adapter attached once at creation per the §7 verdict), and `RuntimeOrchestrator` (the single entry point, building both managers from one shared runtime instance) all landed, with the wiring logic Grok-CLEAN; the real-model success path is still unverified by automated tests. Telemetry/test surfaces now include Ollama runtime health, a ModelDepot local-folder scan, an opt-in Avalonia Settings native smoke tool, explicit "retry with Ollama?" fallback, and a local `.orc/runtime-fallback/` evidence queue. Remaining: SessionManager/AdapterManager-backed telemetry, and actually routing a live call site through this stack instead of Ollama. | Med-High |
| **4** | `OrcScheduler` — capability + VRAM + lane-aware dispatch; pipeline boss→workers. | 🔶 Started — interface + data model + a real VRAM-budget admission check landed; not yet wired into AdapterManager, no live GPU dispatch or pipeline queueing. | High |
