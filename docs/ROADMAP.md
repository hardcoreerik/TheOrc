...
### Installer: cross-platform Avalonia rewrite — Phases 1-2-4 shipped 2026-06-21
Full spec: [`INSTALLER_REVAMP_SPEC.md`](INSTALLER_REVAMP_SPEC.md). `OrchestratorSetup` was a Windows-only WPF wizard (`net10.0-windows`, WMI, `netsh`, `.lnk`, registry) built before the Avalonia migration, capping the whole product to Windows even though the app + daemon are cross-platform. **Phase 1 (UI port + page restructure)**: `OrchestratorSetup` is now `net10.0` Avalonia, ported directly to the final 8-page flow (10 pages → 8: Profile dropped — the app's FirstRunWindow already does `.agent.md` — License folded into Welcome, OllamaCheck replaced by a native-first Runtime Setup page); a "Configure HIVE Ports" button added to Complete. **Phase 2 (`IPlatformInstaller`)**: every OS-coupled action (hardware detection, firewall/HIVE, launchers, uninstall, launch path) now goes through one interface; `WindowsPlatformInstaller` is pure delegation, no rewrite. **Phase 4 (`LinuxPlatformInstaller`)**: `/proc`+`nvidia-smi`/`lspci` detection, XDG paths, `ufw`/`firewalld` via `pkexec`/`sudo`, `.desktop` launchers, manifest-file uninstall registration. All Windows-only service logic carries over unchanged (it was already plain C#, not WPF). **Important caveat, not yet closed**: Phase 4 does NOT make an end-to-end Linux install possible — `InstallerState`'s exe paths, `InstallOrchestrator`'s download/copy logic, the single OS-unaware `app.download_url` manifest key, and `release.yml` (win-x64-only, no Linux publish job or asset) are all untouched and Windows-only; a real Linux install fails at the download step today. Closing that gap is release-engineering work on the main app (`OrchestratorIDE.Avalonia`), explicitly named as Phase 5's job ("Per-OS publish RIDs + delivery wrappers"), not something folded into Phase 4. **Remaining**: Phase 5 — macOS impl + that packaging work for both OSes. Firewall opens HIVE ports automatically with elevation (UAC/sudo/pkexec), never manual-first.
