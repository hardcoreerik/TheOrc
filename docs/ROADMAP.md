...
2. CF-1 deterministic ingestion, structural segmentation, and content-addressed source storage. **Ready to start:** CF-0 no longer blocks implementation; no CF-1 repository, migration, or product ingestion path has shipped yet.
...
### Installer: cross-platform Avalonia rewrite — Phases 1, 2, 4, 5 shipped 2026-06-21 (release pipeline still Windows-only)
Full spec: [`INSTALLER_REVAMP_SPEC.md`](INSTALLER_REVAMP_SPEC.md). `OrchestratorSetup` was a Windows-only WPF wizard (`net10.0-windows`, WMI, `netsh`, `.lnk`, registry) built before the Avalonia migration, capping the whole product to Windows even though the app + daemon are cross-platform. **Phase 1 (UI port + page restructure)**: `OrchestratorSetup` is now `net10.0` Avalonia, ported directly to the final 8-page flow. **Phase 2 (`IPlatformInstaller`)**: every OS-coupled action (hardware detection, firewall/HIVE, launchers, uninstall, launch path) now goes through one interface; `WindowsPlatformInstaller` is pure delegation, no rewrite. **Phase 4 (`LinuxPlatformInstaller`)**: `/proc`+`nvidia-smi`/`lspci` detection, XDG paths, `ufw`/`firewalld` via `pkexec`/`sudo`, `.desktop` launchers, manifest-file uninstall registration. **Phase 5 (`MacPlatformInstaller`)**: `system_profiler`+`uname`+`sysctl` detection, Application Support paths, the per-app Application Firewall via `osascript`-elevated `socketfilterfw` (the GUI-native equivalent of UAC/`pkexec` — raw `sudo` has no TTY to prompt in from a windowed app), a `~/Applications` symlink launcher standing in for a real `.app` bundle. All Windows-only service logic carries over unchanged (it was already plain C#, not WPF). **Important caveat, not yet closed**: none of this makes an end-to-end Linux or macOS install possible yet — see the next entry, scoped for v1.9.5.

### Multi-OS release pipeline — Phases A-D shipped for macOS, 2026-06-21 (Linux not started)
Full spec: [`MULTI_OS_RELEASE_SPEC.md`](MULTI_OS_RELEASE_SPEC.md). Closes the gap the installer entry above left open, for macOS specifically — driven by a real ask (a friend with a Mac, first non-cross-compile test of any of this). `release.yml` now has a `macos-latest`/`osx-arm64` matrix leg alongside the existing Windows one, serialized via a job-level `concurrency` group (two legs racing to create the same GitHub Release tag was a real bug, not theoretical). The manifest's `app.download_url` is OS-keyed; `InstallerState`'s exe/install paths go through `PlatformInstaller.Current`; `LlamaCppResolver` is OS+arch-aware with a real `metal` variant (verified against llama.cpp's actual current release via `gh api`, not guessed); `ZipExtractService` gained `.tar.gz` support; every binary placement now sets the Unix executable bit, which nothing did before. Five review rounds also caught three more spots in the *running app* (`UpdateChecker`, `SelfUpdater`, `LlamaServerManager`) that only ever recognized Windows binary names — fixing only the installer would have shipped a Mac install that completes and then can't update itself or find its own runtime. **No Linux leg yet** — Phase A's matrix has one macOS entry, not two; **Phase E** (Linux/macOS Ollama install) and **Phase F** (real-hardware verification — this has never actually run on a real Mac, only cross-compiled and reviewed) are next. AppImage/`.dmg` packaging and macOS notarization stay explicitly out of scope.
