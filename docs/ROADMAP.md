...
### Multi-OS release pipeline — scoped 2026-06-21, targeting v1.9.5
Full spec: [`MULTI_OS_RELEASE_SPEC.md`](MULTI_OS_RELEASE_SPEC.md). The gap the installer entry above leaves open: `InstallerState`'s exe paths, `InstallOrchestrator`'s download/copy logic, the model manifest's single OS-unaware `app.download_url` key, `release.yml` (win-x64-only, no Linux/macOS publish job or asset), `LlamaCppResolver`'s filename-matching tables (hardcode `"win"` in every variant), and `OllamaInstaller` (silent `.exe`, no Linux/macOS equivalent) are all still Windows-only — a real install on Linux or macOS fails at the download step before reaching any `IPlatformInstaller` code. Scoped into six independently-shippable phases: **A** add Linux/macOS publish jobs to `release.yml`; **B** OS-key the manifest's `app` entry; **C** wire `AppExePath`/`PortableAppExePath` through `PlatformInstaller.Current.LaunchCommand` (closes a gap deferred since Phase 2) plus per-OS archive extraction; **D** OS-aware `LlamaCppResolver` + a `metal` variant for macOS; **E** Linux/macOS Ollama install (lower priority, may slip past v1.9.5 — Ollama is the optional path, native runtime is default); **F** actual verification on real Linux/macOS hardware, not just cross-compile. AppImage/`.dmg` packaging and macOS notarization stay explicitly out of scope — downstream of this, not a prerequisite.
