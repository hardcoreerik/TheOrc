...
| **3** | `ModelDepot` (local registry first; downloader later), `SessionManager` (persistent base model), `AdapterManager` (boss/worker/reviewer LoRAs), telemetry. | 🔶 Started — ModelDepot, SessionManager, and a first Ollama-only telemetry surface landed; the required §7 hot-swap spike is CLOSED with empirical results (real GGUF LoRA + LLamaSharp 0.27 run, not just source-reading) — swapping adapters on a live context is silent and unsafe without an explicit memory clear, confirming separate persistent contexts per role over shared-context swap; AdapterManager implementation itself and SessionManager-backed telemetry still pending. | Med-High |
