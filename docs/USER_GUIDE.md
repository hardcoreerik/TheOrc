...
TheOrc is a desktop app (Avalonia, built for Windows, macOS, and Linux from one codebase — Windows is the primary tested platform) that lets you run AI coding assistants on your own PC. It works with local AI models (no cloud required), keeps you in control of every file change, and supports everything from a single AI helper to a full team of specialized AI workers running across multiple computers. This guide walks you through the main parts of the app.
