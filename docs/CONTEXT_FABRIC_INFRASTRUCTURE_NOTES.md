...
| Qwen3.5-9B (any quant, e.g. Q8_0/Q4_K_M) | Hybrid attention + Gated Delta Net (recurrent); thinking-mode model | **OOM fixed (PR #56), `NoKvSlot` NOT reliably fixed** — the `SeqMax` fix from PR #56 eliminates the deterministic-from-cold-start crash and the OOM it traded for, but two later full-120-question Phase E runs (both quants, identical code) hit a severe, unresolved `NoKvSlot` storm partway through (96 and 59 incidents) — root cause open, see §7e. **Reader-incompatible until PR #58** (thinking-suppression fix) — without it, the model's default reasoning mode consumes the reader's entire completion budget on every segment, so 0/128 segments ever get accepted. **No valid full-scale CF-7 score exists for this model as of 2026-07-15** — every full run so far has been invalidated by either the thinking-suppression bug (§7c) or the §7e storm. | §7b (SeqMax), §7c (thinking suppression), §7e (open `NoKvSlot` storm) |
