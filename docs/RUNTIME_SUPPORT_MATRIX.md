...
**Half-closed as of 2026-07-30** (`NATIVE_RUNTIME_V2_SPEC.md` §5.4): `NativeWithFallbackRuntime` now accumulates a lifetime `FallbackCount`/`LastFallbackReason` — the exact counter this section originally asked for. `HiveWorkerAgent` got the same shape, surfaced over `GET /hive/native-telemetry`. **Still open**: the Context Fabric report writers (`ContextFabricReportWriter.cs`, `ContextFabricBaselineRunner.cs`, `ContextFabricFeasibilityRunner.cs`, `ContextFabricBenchmarkExpansionRunner.cs`/`Writer.cs`) don't yet read the counter or include it in report output — each of the four report record types (`FabricBaselineSystemReport`, `FabricFeasibilityReport`, `FabricCorpusReadReport`, `FabricBoundaryStitchReport`) would need a field alongside `RuntimeName`, populated when `_runtime is NativeWithFallbackRuntime nwf` (e.g. `"runtime": "NativeWithFallback", "fallback_calls": 3, "total_calls": 260`).
