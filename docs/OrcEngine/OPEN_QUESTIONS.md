...
Added following the Python-side streaming/ablation evidence (see [Project Truth](PROJECT_TRUTH.md) and [Decision Log](DECISION_LOG.md) OE-ADR-019) and the `Infinite_Model_Runtime_Claude_Handoff.md` steering review. Originally assigned to Phase 6B, the CPU working-set parts are now proposed Phase-3 questions because Phase 2 implemented real file-backed extents and F32 execution earlier than planned. Device-tier planning remains later work -- renumbered from Phase 6B to Phase 7B in the reconciled roadmap (Phase 6 itself was reassigned to initial quantization).

- OQ-041: What is the right default granularity for temporary `ResidentView` values in the real C++ engine -- whole layer, individual tensor, or tensor tile? **Proposed Phase-3 answer pending evidence:** whole layer is the mandatory first reference; one narrower experiment is selected only if measured bookend/layer/tensor residency justifies it. Tile paging remains deferred.
...
- OQ-043: What is the actual byte-amortization/throughput cost of true layer streaming in a real (non-research-harness) inference loop generating many tokens, not a one-shot ablation sweep? **Promoted into the proposed Phase-3 measurement gate:** record materializations, bytes read, repeated reads, and time for one and four full-prefix steps. KV cache and speculative decoding remain later work; Phase 3 must report their absence rather than hide the cost.
