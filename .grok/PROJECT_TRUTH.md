...
| 3 | ModelDepot + SessionManager + AdapterManager (boss/worker/reviewer) + telemetry | 🔶 In progress — local-only ModelDepot registry and SessionManager base-load coordinator landed; first telemetry surface landed (Settings panel shows `OllamaRuntime.GetHealth()/GetStats()` — Ollama only, not SessionManager-driven, since `OllamaRuntime` doesn't implement `ILocalModelRuntime`); AdapterManager and SessionManager-backed telemetry (once a local runtime is wired into the live chat path) pending |
