...
public partial class SettingsPanel : UserControl
{
    ...
    /// <summary>Fired when the user clicks "Check Now".</summary>
    public event Func<Task>? CheckUpdatesRequested;

    /// <summary>Fired when the user clicks "Regenerate Agent File".</summary>
    public event Func<Task>? RegenerateAgentFileRequested;

    /// <summary>
    /// Fired when the user clicks "Open in Agent" — MainWindow loads the
    /// source folder as the active workspace.
    /// </summary>
    public event Action<string>? OpenFolderAsWorkspaceRequested;

    /// <summary>
    /// Fired when Scan completes — MainWindow injects the analysis prompt
    /// into the active agent session so the user can continue the conversation.
    /// </summary>
    ...
    public SettingsPanel(OllamaClient ollama)
    {
        ...
        // Show install folder path immediately
    ...
    public void LoadSettings(AppSettings s)
    {
        ...
        // Source folder — default to %AppData%\OrchestratorIDE\source
        ...
        // Update "open source in agent" button state
    ...
    private void BtnSetPermanent_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            SetStatus(
                $"✓  OLLAMA_NUM_PARALLEL={slots} written. Restart Ollama to apply.",
                "#76B900");
            RefreshParallelStatus();
        }
    ...
    {
        var dlg = new OpenFolderDialog { Title = "Choose folder for TheOrc source code" };
        if (dlg.ShowDialog() == true)
        {
            TbSourceFolder.Text = dlg.FolderName;
            RefreshSourceButtons();
        }
    }

    // ── Grab Source ───────────────────────────────────────────────────────────
    // Clones https://github.com/hardcoreerik/TheOrc into the configured folder,
    // or runs "git pull" if a clone already exists there.
    ...
    private async void BtnGrabSource_Click(object sender, RoutedEventArgs e)
    {
        ...
        else
        {
            ...
            // Clone into the folder itself (not a sub-directory)
    ...
    // Fetches open issues + recent commits from the GitHub API, then builds a
    // structured analysis prompt and fires it via ScanAnalysisReady so
    // MainWindow can inject it into the active agent session.
        ...
        List<GitHubCommit>? commits)
    {
        ...
        if (issues?.Count > 0)
        {
            ...
            foreach (var iss in issues)
            {
                ...
                {
                    // Trim body to ~150 chars to keep prompt compact
    ...
    private void SetSelfStatus(string msg, string hex)
    {
        ...
        TbSelfImproveStatus.Foreground = new SolidColorBrush(
            (Color)ColorConverter.ConvertFromString(hex));
    }
    ...
    private record GitHubIssue(int Number, string Title, string Body,
        List<string> Labels, string HtmlUrl);
