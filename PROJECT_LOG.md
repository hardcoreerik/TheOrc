...
**GOBLIN MIND CLI Backend** (`tool-probe.exe` — `Tools/ToolCallTester/Program.cs`)
- Full rewrite from ~560 lines (dispatch-only) to ~900 lines with all GOBLIN MIND subcommands
- New subcommands: `format`, `categories`, `full`, `evolve`, `list`
- Backward compat: no-subcommand defaults to `dispatch`, `--list` flag still works
- All subcommands support `--json` output + write to shared `tool-call-profiles.json`
- `ProfileStore.MutateAsync` pattern — partial non-destructive updates per probe type
- `ExtractToolCall(msg, fmt)` unified helper; format parsers: Hermes XML, Python-style, YAML, bare JSON
- Schema mutation helpers: `MutateTerseDesc`, `MutateReqFirst` (DeepClone)
- **Bugs fixed during session:** CS8803 (type decls inside top-level statements) → moved all `record` types to bottom; CS8210 (void in tuple switch expression) → converted to switch statement

**Settings Panel Overhaul** (`SettingsPanel.xaml/.cs`)
- Fixed ALL toggle DockPanel ordering — toggle Dock="Right" must be FIRST child in WPF DockPanel
- Added TextWrapping="Wrap" to all hint TextBlocks
- New INSTALL section: install path display, 📁 Open Folder, 🗂 Data Folder buttons
- New SELF-IMPROVE section: source folder textbox + browse, ⬇ Grab Source / ↺ Pull Latest, 📂 Open in Agent, 🔍 Scan GitHub
- `RefreshSourceButtons()` — detects `.git` dir to toggle clone vs pull label
- `FetchGitHubDataAsync()` — issues + commits API, partial data on failure
- `BuildScanPrompt()` — structured markdown asking model to prioritize bugs, identify improvements, suggest code changes
- `RunGitAsync()` — `Process.Start("git", args)` with redirected output
- Two new events: `OpenFolderAsWorkspaceRequested`, `ScanAnalysisReady`

**MainWindow.xaml.cs**
- Wired `OpenFolderAsWorkspaceRequested` → `OpenWorkspaceAsync` → `ConfirmWorkspace`
- Wired `ScanAnalysisReady` → opens Agent panel → `InjectUserMessage` (pre-fills, no auto-send)

**AgentPanel.xaml.cs**
- Added `InjectUserMessage(prompt)` — pre-fills input textbox, focuses, moves caret to end

**AppSettings.cs**
- Added `SourceFolderPath` field under Self-improve section

**MainWindow.xaml**
- Menu bar row: `Height="22"` → `Height="26"`
- Sidebar MinWidth: 140 → 200
- Status bar row: `Height="24"` → `Height="30"`
- All status bar text: 10–11pt → 12–13pt
- Screenshot button: 11pt → 15pt, wider padding
- Trust pill style (App.xaml): FontSize 10 → 12, Padding 7,3 → 8,4

**Build result:** ✅ 0 errors, 11 warnings (all pre-existing: SharpAvi compat + CS8602 null-ref in SwarmSession.cs)
