...
Current accurate status always lives in `.grok/PROJECT_TRUTH.md` — check it before
claiming something is "done" or "not started."
...
2. Check `.grok/PROJECT_TRUTH.md` for current accurate status before assuming a feature is or isn't done.
...
8. Update `.grok/PROJECT_TRUTH.md` if your change closes a known gap or changes a status line — don't let docs drift from reality.
