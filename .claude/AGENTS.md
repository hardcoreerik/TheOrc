...
| 3 | ModelDepot + SessionManager + AdapterManager (boss/worker/reviewer) + telemetry | 🔶 In progress — ModelDepot, SessionManager, and first telemetry surface (Ollama only) landed; §7 hot-swap spike CLOSED — separate persistent contexts per role confirmed empirically, see `RUNTIME_PHASE0_SPEC.md` §7; AdapterManager implementation and SessionManager-backed telemetry pending |
