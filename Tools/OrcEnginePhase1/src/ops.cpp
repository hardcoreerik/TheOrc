...
// combination (e.g. rows=-1 paired with out_features=INT64_MIN through
// the return-by-value rmsnorm()/linear_no_bias() wrappers, which call
// this function directly before check_positive_dim ever runs -- see
// DECISION_LOG.md OE-ADR-041/OE-ADR-042 for the hand-traced proof), so
...
std::vector<float> embedding_lookup(const std::vector<float>& table, int64_t hidden,
    ...
    // has its own added check for this same gap -- see forward_cached.cpp).
