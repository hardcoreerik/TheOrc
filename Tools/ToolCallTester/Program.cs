...
var exportMd    = cliArgs.Contains("--export-md");
var exportPath  = ArgValue(cliArgs, "--out") ?? "MODEL_PROFILES.md";
...
if (subcommand == "list")
{
    ...
    if (exportMd)
    {
        var md = BuildModelProfilesMd(stored);
        File.WriteAllText(exportPath, md);
        C($"✓ MODEL_PROFILES.md written to {exportPath}", ConsoleColor.Green);
        C($"  {stored.Count} model profile(s) exported.", ConsoleColor.DarkGray);
        return 0;
    }
    ...
    C("Tip: add --export-md to write MODEL_PROFILES.md", ConsoleColor.DarkGray);
    return 0;
}
...
static string BuildModelProfilesMd(List<StoredProfile> profiles)
{
    var sb = new StringBuilder();
    sb.AppendLine("# MODEL_PROFILES.md");
    sb.AppendLine($"> Auto-generated by `tool-probe list --export-md` on {DateTime.Now:yyyy-MM-dd}");
    sb.AppendLine();
    sb.AppendLine("Capability profiles for all locally probed models.");
    sb.AppendLine("Run `tool-probe full --model <name>` to update a profile.");
    sb.AppendLine();

    // Summary table — ordered by native pass count descending (best boss candidates first)
    sb.AppendLine("## Summary Table");
    sb.AppendLine();
    sb.AppendLine("| Model | Dispatch Mode | Format | Native/Total | Categories | Last Probed |");
    sb.AppendLine("|---|---|---|---|---|---|");
    foreach (var p in profiles.OrderByDescending(p => p.NativePasses).ThenBy(p => p.ModelId))
    {
        var categories = p.CategorySummary ?? "—";
        var lastProbed = p.TestedAt != default ? p.TestedAt.ToString("yyyy-MM-dd") : "—";
        var passRatio  = p.TotalTests > 0 ? $"{p.NativePasses}/{p.TotalTests}" : "—";
        sb.AppendLine($"| `{p.ModelId}` | {p.RecommendedMode} | {p.PreferredFormat ?? "?"} | {passRatio} | {categories} | {lastProbed} |");
    }
    sb.AppendLine();
    ...
    foreach (var p in profiles.OrderByDescending(p => p.NativePasses).ThenBy(p => p.ModelId))
    {
        sb.AppendLine($"### `{p.ModelId}`");
        sb.AppendLine();
        sb.AppendLine($"- **Dispatch mode:** {p.RecommendedMode}");
        sb.AppendLine($"- **Preferred format:** {p.PreferredFormat ?? "not probed"}");
        sb.AppendLine($"- **Native passes:** {p.NativePasses} / {p.TotalTests}");
        sb.AppendLine($"- **Text-JSON passes:** {p.TextPasses} / {p.TotalTests}");
        sb.AppendLine($"- **Last probed:** {(p.TestedAt != default ? p.TestedAt.ToString("u") : "never")}");

        if (!string.IsNullOrWhiteSpace(p.CategorySummary))
            sb.AppendLine($"- **Categories:** {p.CategorySummary}");

        if (p.FormatScores?.Count > 0)
        {
            sb.AppendLine();
            sb.AppendLine("  **Format scores:**");
            sb.AppendLine();
            sb.AppendLine("  | Format | Score |");
            sb.AppendLine("  |---|---|");
            foreach (var kv in p.FormatScores.OrderByDescending(x => x.Value))
                sb.AppendLine($"  | {kv.Key} | {kv.Value} |");
        }

        if (p.CategoryResults?.Count > 0)
        {
            sb.AppendLine();
            sb.AppendLine("  **Category results:**");
            sb.AppendLine();
            sb.AppendLine("  | Category | Result |");
            sb.AppendLine("  |---|---|");
            foreach (var kv in p.CategoryResults.OrderBy(x => x.Key))
                sb.AppendLine($"  | {kv.Key} | {kv.Value} |");
        }
    ...
    sb.AppendLine("---");
    sb.AppendLine($"*Generated {DateTime.UtcNow:u} UTC — {profiles.Count} profile(s)*");
    return sb.ToString();
}
...
static void PrintHelp()
{
    ...
    Options:
      ...
      --export-md          (list) Write MODEL_PROFILES.md instead of console output
      --out   <path>       (list --export-md) Output path (default: MODEL_PROFILES.md)
      --generations N      (evolve) Number of mutation generations (default: 3)
      --tool  <name>       (evolve) Restrict to this tool name
    ...
    Examples:
      ...
      tool-probe list --export-md              Write MODEL_PROFILES.md
      tool-probe list --export-md --out docs/MODEL_PROFILES.md
