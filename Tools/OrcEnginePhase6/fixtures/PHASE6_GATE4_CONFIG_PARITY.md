...
| tied/untied output weights | `tie_word_embeddings` = `true` | `output.weight` tensor IS present as a separate tensor (`output_semantics: untied` per `orcengine_gguf_inspect`) | `tied_embeddings = !output_present` = **false (untied)** | untied (llama.cpp materializes the GGUF's actual `output.weight` tensor if present, same as OrcEngine; already independently confirmed via the paired-report `llama_f32_top5`/`llama_q8_top5` data using the real lm_head, not a tied fallback) |
