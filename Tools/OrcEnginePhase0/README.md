...
| `three_way_oracle_independence` | Partial, strengthened — hand-derived leg (microcases.py) done; primary semantic oracle leg now cross-validated by TWO independent implementations (`oracle/model.py` NumPy, `oracle/torch_oracle.py` PyTorch, re-derived from the spec independently) agreeing to max_abs_diff=2.4e-07 on both n_layers=1 and n_layers=2 configs (`oracle/cross_oracle_check.py`). **Still not a pass**: the secondary deployment oracle (pinned llama.cpp) leg hasn't started. Promising path: Profile A's block semantics (RMSNorm/GQA/RoPE/SwiGLU) ARE the standard "llama" GGUF architecture llama.cpp already supports at any size — writing Profile A's own weights into a real "llama"-architecture GGUF (no real model needed) could close this leg without waiting for Fixture D. |
