...
def load_config(config_path: Path) -> dict:
    ...
    for key in ("foundry_track", "status", "base_model", "adapter", "dataset", "hyperparams", "gates", "output"):
        if key not in cfg:
...
def run_preflight(cfg: dict, repo_root: Path = REPO_ROOT) -> list[str]:
    ...
    for name, rows in (("train", train_rows), ("eval", eval_rows)):
        bad = sum(1 for r in rows if not isinstance(r.get("messages"), list) or len(r["messages"]) < 2)
        if bad:
    ...
    if expected_hash:
        ...
        if meta and meta.get("tool_schema_hash") and meta["tool_schema_hash"] != expected_hash:
            findings.append("SCHEMA: dataset was exported under a different frozen inventory hash")
        ...
        if meta and isinstance(meta.get("lineage_groups"), dict):
            ...
            if overlap:
                findings.append(f"LEAKAGE: {len(overlap)} lineage group(s) present in both splits: "
                                f"{sorted(overlap)[:5]}")
        def _answers(rows):
            return {m["content"] for r in rows if isinstance(r.get("messages"), list)
